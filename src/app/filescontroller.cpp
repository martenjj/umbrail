...
FilesController::Status FilesController::importFile(const QUrl &importFrom)
{
    ...
    const bool isLoad = (model()->isEmpty());		// any data held so far?

    // If the imported file has categories defined, then merge them with
    // existing categories if there are any.  It could be argued that this
    // should be done below as an undoable action, but that would complicate
    // things because the category map generated by the importer would have
    // to be copied safely somewhere and retained in the undo stack.  Although
    // the categories list is application-global, existing categories are not
    // overwritten here unless this is a file load (not for an import), so no
    // destructive changes will be made.
    const QMap<QString, QColor> *catMap = imp->categoryMap();
    if (catMap!=nullptr)
    {
        CategoriesManager *catMgr = CategoriesManager::self();
        for (QMap<QString, QColor>::const_iterator it = catMap->constBegin(); it!=catMap->end(); ++it)
        {
            catMgr->addCategory(it.key(), it.value(), isLoad);
        }

        qDebug() << "have" << catMgr->allCategories().count() << "categories";
    }
