...
QString GarminIconProvider::displayName() const
{
    return (i18n("Garmin"));
}
...
bool GarminIconProvider::createIcon(QIcon *icon, const QString &name,
                                    const QVariant &colour, const QVariant &shape)
{
    ...
    if (idx==-1) return (false);			// name was not found

#ifdef DEBUG_GARMIN
    qDebug() << "  found at index" << idx;
#endif // DEBUG_GARMIN
    setGarminPixmap(icon, idx);				// generate and store pixmaps
...
QStringList GarminIconProvider::allIconNames()
{
    QStringList result;
