                               ...
                               const QString &qName, const QXmlAttributes &atts)
{
    ...
    if (localName=="gpx")				// start of a GPX element
    {
        ...
        if (i>=0) mDataRoot->setMetadata(DataIndexer::indexWithNamespace(atts.qName(i)), atts.value(i));
    ...
    else if (localName=="link")				// start of a LINK element
    {
        ...
        if (!link.isEmpty()) mCurrentPoint->setMetadata(DataIndexer::indexWithNamespace(qName), link);
...
// handle end of element only if it was error free
    ...
    if (localName=="ele")				// end of an ELE element
    {
        ...
        if (p!=nullptr) p->setMetadata(DataIndexer::indexWithNamespace(qName), QVariant(ele));
    ...
    {							// may belong to any element
        ...
        if (item!=nullptr) item->setMetadata(DataIndexer::indexWithNamespace(qName), dt);
    ...
    {							// may belong to any container
        ...
        else if (mWithinMetadata) mDataRoot->setMetadata(DataIndexer::indexWithNamespace(qName), elementContents());
    ...
    else if (localName=="category")			// end of a CATEGORY element
    {
        ...
        if (item!=nullptr) item->setMetadata(DataIndexer::indexWithNamespace(qName), elementContents());
    ...
    else if (localName=="type")				// end of a TYPE element
    {
        ...
        if (dynamic_cast<TrackDataWaypoint *>(item)!=nullptr)
        {
            ...
            int idx = DataIndexer::index("category");
        ...
        else if (dynamic_cast<TrackDataTrack *>(item)!=nullptr || dynamic_cast<TrackDataSegment *>(item)!=nullptr)
        {
            ...
            item->setMetadata(DataIndexer::indexWithNamespace(qName), elementContents());
        }
    ...
    else						// Unknown tag, save as metadata
    {
        ...
        int idx = DataIndexer::indexWithNamespace(key);
